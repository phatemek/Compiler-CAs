...
patternMatching returns [PatternDeclaration patternRet]://TODO:cunstruct patterDeclaration node
    ...
    RPAR
    {
    Identifier name = new Identifier($patternName.text);
    name.setLine($patternName.line);
    Identifier target = new Identifier($targetVar.text);
    target.setLine($targetVar.line);
    $patternRet = new PatternDeclaration(name, target);
    $patternRet.setLine($pat.line);
    }
    (PATTERN_MATCHING_SEPARATOR c = condition
    {
        for(int i = 0; i < $c.conditionRet.size(); i++) {
            $patternRet.addCondition($c.conditionRet.get(i));
        }
    }
...
forStatement returns [ForStatement forStRet]://TODO:construct forStatement node
    ...
    l = loopBody
    {
    $forStRet = new ForStatement(id, $r.rangeRet, $l.loopExps, $l.loopStmts, $l.loopRetStmt);
...
matchPatternStatement returns [MatchPatternStatement matchPatRet]://TODO:construct match pattern node
    ...
    Identifier id = new Identifier($id.text);
    id.setLine($id.line);
    $matchPatRet = new MatchPatternStatement(id, $e.expRet);
...
expression returns [Expression expRet]:
    ...
    if($e1.expRet instanceof AppendExpression) {
        appendee = ((AppendExpression)$e1.expRet).getAppendee();
        appendeds = ((AppendExpression)$e1.expRet).getAppendeds();
    }
    else {
        appendee = $e1.expRet;
    }
    $expRet = new AppendExpression(appendee);
    ((AppendExpression)$expRet).setAppendeds(appendeds);
    $expRet.setLine($a.line);
    ((AppendExpression)$expRet).addAppendedExpression($e2.expRet);
    }
